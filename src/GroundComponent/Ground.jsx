...
const Ground = () => {
  ...
  const retrieveAllSlots = async () => {
    const response = await axios.get(
      `${API_BASE_URL}/api/book/ground/fetch/slots`
    );
  ...
  const retrieveGround = async () => {
    const response = await axios.get(
      `${API_BASE_URL}/api/ground/id?groundId=` + groundId
    );
  ...
  const bookGround = (e) => {
    ...
    if (user) {
      fetch(`${API_BASE_URL}/api/book/ground/`, {
        method: "POST",
        headers: {
      ...
      alert("Please log in as customer to book the Turfs!!!");
    }
  };
      ...
      <div class="row">
        <div class="col-sm-3 mt-2">
          <div class="card form-card border-color custom-bg">
            <img
              src={`${API_BASE_URL}/api/ground/` + ground.image}
              style={{
                maxHeight: "500px",
                maxWidth: "100%",
                width: "auto",

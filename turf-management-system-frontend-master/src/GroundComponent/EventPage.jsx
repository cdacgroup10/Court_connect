...
const EventPage = () => {
  ...
  const retrieveEvent = async () => {
    const response = await axios.get(
      "http://localhost:8080/api/event/id?eventId=" + eventId
    );
  ...
  const participateEvent = (e) => {
    ...
    if (user) {
      fetch("http://localhost:8080/api/event/customer/participate", {
        method: "POST",
        headers: {
      ...
      <div class="row">
        <div class="col-sm-3 mt-2">
          <div class="card form-card border-color custom-bg">
            <img
              src={"http://localhost:8080/api/ground/" + event.ground.image}
              style={{
                maxHeight: "500px",
                maxWidth: "100%",
                width: "auto",
